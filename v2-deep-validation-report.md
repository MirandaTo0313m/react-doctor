...
- `packages/react-doctor-v2/src/core/diagnostics.ts`: filters React `no-unknown-property` noise for Satori/OG JSX, Emotion `css` prop projects, and styled-jsx while reusing cached source reads for inline-disable and RN text suppression.
- `packages/react-doctor-v2/src/core/rules/lint/nextjs/nextjs-no-img-element.ts`: skips SVG/icon/logo/mark/badge sources that do not benefit from Next image optimization.
...
- `packages/react-doctor-v2/src/core/rules/lint/security/no-secrets-in-client-code.ts`: preserves the test/fixture and URL literal suppression with typed empty visitors, avoiding visitor-shape type errors.
...
- `react-hooks-js/todo` (16 FP):
  - `Fanzzzd/repo-wizard` `src/hooks/usePromptGenerator.ts:191`: This is a React Compiler internal TODO/unsupported lowering diagnostic; the inspected code may be valid application code and not an actionable project bug.
  - `Fanzzzd/repo-wizard` `src/hooks/usePromptGenerator.ts:225`: This is a React Compiler internal TODO/unsupported lowering diagnostic; the inspected code may be valid application code and not an actionable project bug.
  - `Fanzzzd/repo-wizard` `src/hooks/useReviewSession.ts:31`: This is a React Compiler internal TODO/unsupported lowering diagnostic; the inspected code may be valid application code and not an actionable project bug.
...
- `react-hooks-js(todo)` (9 FP):
  - `DefiLlama/defillama-app` `src/containers/LlamaAI/index.tsx:2126`: Compiler 'BuildHIR::lowerStatement Handle TryStatement with finalizer' and 'lowerAssignment Handle computed properties in ObjectPattern' notices. These are React Compiler internal limitations, not user defects. Severity 'error' overstates t
  - `Expensify/App` `src/pages/ReimbursementAccount/USD/ConnectBankAccount/components/BankAccountValidationForm.tsx:72`: All 24 hits are React Compiler internal limitations ('Handle tagged template with interpolations', 'Handle TryStatement with finally', 'Handle YieldExpression', 'lowerReorderableExpression CallExpression'). NOT user-fixable defects — these 
  - `Uniswap/interface` `src/hooks/useUniswapXSwapCallback.ts:206`: 'Todo: (BuildHIR::lowerStatement) Support ThrowStatement inside of try/catch' is a React Compiler internal lowering bailout. It is not an actionable user-fixable bug.
- `oxlint/unknown` (9 FP):
