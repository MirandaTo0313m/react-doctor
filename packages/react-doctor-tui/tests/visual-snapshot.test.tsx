...
describe("visual snapshots", () => {
  it("logs a populated dashboard frame", () => {
    const { lastFrame } = render(<DashboardView state={buildPopulatedState()} />);
  ...
  it("logs a populated review frame", () => {
    const { lastFrame } = render(<ReviewView state={buildPopulatedState()} />);
    const frame = lastFrame() ?? "";
    if (process.env.SNAPSHOT_LOG === "1") {
      process.stdout.write(`\n===== REVIEW =====\n${frame}\n==================\n`);
    }
