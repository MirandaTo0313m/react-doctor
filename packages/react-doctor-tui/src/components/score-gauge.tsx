...
const renderDeltaBadge = (deltaValue: number): React.ReactElement => {
  if (deltaValue === 0) {
...
export const ScoreGauge = ({
  ...
  if (score === null) {
    return (
      <Box flexDirection="column">
        ...
        <Text color="gray">{"░".repeat(SCORE_BAR_WIDTH_CHARS)}</Text>
        <Text color="gray">no score available</Text>
      </Box>
    );
  }
  const segments = buildScoreBarSegments(tweenedScore);
  ...
  return (
    <Box flexDirection="column">
      <Box>
        ...
        {showDelta ? (
          <>
            ...
            <Text color="gray"> since last scan</Text>
          </>
        ) : null}
      </Box>
      <Box>
      ...
      {isOffline ? <Text color="gray">offline mode — score calculated locally</Text> : null}
    </Box>
  );
};
