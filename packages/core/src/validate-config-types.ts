...
const surfaceControlsSchema = (surface: DiagnosticSurface) =>
  z.object(
    Object.fromEntries(
      SURFACE_CONTROL_FIELDS.map((controlField) => [
        controlField,
        filteringStringArraySchema(`surfaces.${surface}.${controlField}`).optional(),
      ]),
    ),
  );
...
const extendsSchema = z.union([
  ...
  z
    ...
    .transform(
      filteringArrayTransform(
        isNonEmptyString,
        () => `config field "extends" contains a non-string entry; ignoring the entry.`,
      ),
    ),
]);
...
const fieldDescriptors = {
  ...
  extends: {
    expectedDescription: "a string or array of strings",
